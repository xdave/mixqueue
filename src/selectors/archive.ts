...
export const getMixById = createSelector(_getMixById, (mix) => mix);
...
export const getTracks = createSelector(_getMixById, (mix) =>
  mix ? mix[mix.metadata.identifier].tracks : []
);

export const getMixes = createSelector(getState, (archive) =>
...
export const getCurrentTrack = createSelector(
  ...
  (tracks, { currentTime }): Track =>
    tracks.reduce(
      (prev, cur) =>
        currentTime >= prev.time && currentTime < cur.time ? prev : cur,
